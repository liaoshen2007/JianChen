...
namespace Assets.Scripts.Module.Components.List.EasyObjectPool
{
    ...
    public class ResourceManager : MonoBehaviour
    {
        ...
        public void InitPool(string poolName, int size, PoolInflationType type = PoolInflationType.DOUBLE)
        {
            ...
            {
                GameObject pb = Framework.JianChen.Service.ResourceManager.Load<GameObject>("module/"+poolName);
                if (pb == null)
                {
        ...
        public void DestroyPool(string poolName)
        {
            ...
            {
                Debug.LogWarning("[ResourceManager] Invalide poolName name:" + poolName);
                return;
            }
            poolDict[poolName].DestroyPool();
...
//            Recursive(t);
        ...
        private static void Recursive(Transform transform)
        {
            foreach (Transform child in transform)
            {
                ...
                if (img != null)
                {
                    string _name = img.sprite.name.ToLower();
                    if (_name.Contains("uiatlas_common"))
                    {
                        Debug.LogError("");
                    }
                }
